...
public class PorterStemmer {
	...
	private final static String c = "(?:[^aiueoy]|(?:(?<=[aiueo])y)|\by)"; //reconoce una consonante
	
	//This is a vowel. "aiueo" and "y" if preceded by a consonant
	private final static String v = "(?:[aiueo]|(?:(?<![aiueo])y))";   //reconoce una vocal
	...
	private StringWrapper step1( StringWrapper w ) {
		...
		} else if ( m1a_3.find() ){              // engloba 2 ultimas reglas de 1a
			w.word = m1a_3.group(1);               // Same as last 2 rules of 1a
		}
	...
	private StringWrapper step4( StringWrapper w ) {
		...
		if( letter.equals("a") ){
			...
			if ( m_ion.find() ) {
				if ( p_st.matcher(m_ion.group(1)).find() &&
						m_gt_1.matcher(m_ion.group(1)).find() ) {
	...
	private StringWrapper step5( StringWrapper w ) {
		...
		if( m5_1.find() ) {
			if ( m_gt_1.matcher( m5_1.group(1) ).find() ||
				(m_eq_1.matcher( m5_1.group(1) ).find() &&
				 pO.matcher( m5_1.group(1) ).find() ) ) {
				w.word = m5_1.group(1);
			}
		}
