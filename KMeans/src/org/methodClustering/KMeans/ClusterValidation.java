...
public class ClusterValidation {
	...
	private static List<Cluster> machineClusters;		//clusters generated by the algorithm
	...
	public static void runClusterValidation(List<Cluster> clusters, List<List<String>> mTags){
	...
	private static void setClusterObjCount(){
		...
		for(int i=0; i<machineClusters.size(); i++){
			methodsIDs = machineClusters.get(i).methodsIDs;
			for(int Clus : methodsIDs){
...
//	}	
	...
	private static void printClusterDetails(){
		...
		for(int i=0; i<machineClusters.size(); i++){
			...
			for(int j=0; j<humanClusters.size(); j++){
				...
				for(int clus: machineClusters.get(i).methodsIDs){
					if(humanClusters.get(j).contains(clus)){
						System.out.println(clus + " "+ projectDict.get(clus)+" "+methodsTags.get(clus));
					}
				}
				System.out.println();
			}	
	...
	private static void populateHumanClusters(){	
		...
		clusterName = new String[]{"BoardGames","RPG ACTION","Card Games"};
		//boardGames
		humanClusters.add(Arrays.asList(0,1,2,3,4,5,6,7,8,11,12,14,15,16,17,20,21,24,29,32));
		//RPG Action
		humanClusters.add(Arrays.asList(9,10,13,22,25,26,31));
		//Card Games
		humanClusters.add(Arrays.asList(18,19,23,27,28,30));
